...
if( $action != '' ) {
	...
	Page_Terminate("postslistsorted.php");
}
...
if(isset($_GET['lastwhere'])){
	$lastwhere = @$_GET['lastwhere'];
}
if(isset($_GET['maxmatch'])){
	$maxmatch = $_GET['maxmatch'];
	$maxmatch = intval($maxmatch);
}else{
	$maxmatch=0;
}
if ($sSrchBasic <> "") {
...
// Set Return Url
$posts->setReturnUrl("postslistsorted.php");
?>
...
<?php } ?>
<?php
if(isset($_GET['fsecond'])&&($_GET['fsecond']=='on')){
// Load recordset
$bExportAll = (defined("EW_EXPORT_ALL") && $posts->Export <> "");
$bSelectLimit = ($posts->Export == "" && $posts->SelectLimit);
if (!$bSelectLimit) $rs = LoadRecordsetInSecond($lastwhere);
$nTotalRecs = ($bSelectLimit) ? $posts->SelectRecordCount() : $rs->RecordCount();
$nStartRec = 1;
if ($nDisplayRecs <= 0) $nDisplayRecs = $nTotalRecs; // Display all records
if (!$bExportAll) SetUpStartRec(); // Set up start record position
if ($bSelectLimit) $rs = LoadRecordsetInSecond($nStartRec-1, $nDisplayRecs,$lastwhere);
}else
{
// Load recordset
$bExportAll = (defined("EW_EXPORT_ALL") && $posts->Export <> "");
$bSelectLimit = ($posts->Export == "" && $posts->SelectLimit);
if (!$bSelectLimit) $rs = LoadRecordset($lastwhere);
$nTotalRecs = ($bSelectLimit) ? $posts->SelectRecordCount() : $rs->RecordCount();
$nStartRec = 1;
if ($nDisplayRecs <= 0) $nDisplayRecs = $nTotalRecs; // Display all records
if (!$bExportAll) SetUpStartRec(); // Set up start record position
if ($bSelectLimit) $rs = LoadRecordset($nStartRec-1, $nDisplayRecs,$lastwhere);
}
...
<form name="fpostslistsrch" id="fpostslistsrch" action="postslistsorted.php" >
<div class="menuhead">实时检索</div>
<?php
	$keywords = trim($posts->getBasicSearchKeyword());
	$arKeyword = explode(" ", trim($keywords));
	$sql = "";
	foreach($arKeyword as $sKeyword){
		$sKeyword = ew_AdjustSql($sKeyword);
		if($sKeyword <> '')
		$sql .= "`content` LIKE '%" . $sKeyword . "%' OR ";
	}
	if (substr($sql, -4) == " OR ") $sql = substr($sql, 0, strlen($sql)-4);
	$arKeyword = explode(" ", trim($lastwhere));
	if(trim($lastwhere) <> '')
		$sql = $sql.' AND '.$lastwhere;
	echo '<input type="hidden" name="lastwhere" value="'.$sql.'" />';
?>	
...
<table class="ewBasicSearch">
	...
	<tr>
		<td>
			<label >隐藏匹配次数小于</label>
		</td>
		<td>
			<input type="text" value="<?php echo $maxmatch?>"  size="6" id="maxmatch" name="maxmatch"/>的记录</p>			
		</td>
	</tr>	
	<tr>
		<td colspan="2">
	...
	<tr>
		<td colspan="2">
			<input type="checkbox" name="fsecond" >在结果中搜索&nbsp;</br>
...
while( !$crs->EOF && $crs->fields('projectname') != '' ) {
	echo '<li>';
	echo '<a href="postslistsorted.php?projectname=' . rawurlencode($crs->fields('projectname')) . '">' . $crs->fields('projectname') . '</a>';
	echo '</li>';
	$crs->MoveNext();
}
?>
...
?>
<p class="ptitle"><b>中和集成监控系统(支持搜索结果统计排序,不支持分页,仅显示搜索结果)</b><span><a href="<?php echo $classicUrl ?>">传统视图</a><a href="<?php echo $normalUrl ?>">默认视图</a></span></p>
<?php
...
?>
<?php  if ($nTotalRecs > 0) { ?>
<form method="post" name="fpostslist" id="fpostslist" action="postslistsorted.php">
<script type="text/javascript">
...
<?php } ?>
		...
		while (!$rs->EOF && $nRecCount < $nStopRec) {
			...
			$postsvalue = array(
			'countmatch'=>count_word( $posts->content->ViewValue,$keywords),
			'id'=>$posts->id->ViewValue,
			'datetime'=>$posts->datetime->ViewValue,
			'url'=>$posts->url->ViewValue,
			'projectname'=>$posts->projectname->ViewValue,
			'title'=>$posts->title->ViewValue,
			'content'=>$posts->content->ViewValue
			);			
			array_push($postsvalues,$postsvalue);
			$rs->MoveNext();
		}
		usort($postsvalues,"mysort");
		foreach($postsvalues as $values){	
			$countmatch=intval($values['countmatch']);
			if(($countmatch==0)||($countmatch<=$maxmatch)){ continue;}
		?>
			<!-- Table body -->
			<tr>
				<td>
					<div><input type="checkbox" name="ids[]" value="<?php echo $values['id'] ?>"/><a href="<?php echo $values['url'] ?>" target="_blank"><?php echo $values['title'] ?></a><?php echo countword( $values['content'],$keywords) ?></div>
				</td>
			</tr>
			<tr>
				<td>
					<div style="color:#000">  <?php echo highlight(substr($values['content'],0,800),$keywords) ?>   </div>
				</td>
			</tr>
			<tr>
				<td>
					<span style="color:green"><?php echo (isset($url_info['host'])?$url_info['host']:'') . (isset($url_info['path'])?$url_info['path']:'') . '&nbsp;&nbsp;' . $values['datetime'] ?></span>
					 - <a href="<?php echo $viewpath . '/' . (isset($url_info['host'])?$url_info['host']:'') . '/' . md5(strtolower($values['url'])) ?>.html" target="_blank">快照</a>
				</td>
			</tr>
			<tr>
				<td colspan="2">
...
<?php } ?>
		...
		id
		<?php } else { ?>
			<a href="postslistsorted.php?order=<?php echo urlencode('id') ?>&ordertype=<?php echo $posts->id->ReverseSort() ?>">编号<?php if ($posts->id->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($posts->id->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
		<?php } ?>
				</td>
				<td valign="top">
		<?php if ($posts->Export <> "") { ?>
		来源
		<?php } else { ?>
			<a href="postslistsorted.php?order=<?php echo urlencode('url') ?>&ordertype=<?php echo $posts->url->ReverseSort() ?>">站点名称&nbsp;(*)<?php if ($posts->url->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($posts->url->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
		<?php } ?>
				</td>
				<td valign="top">
		<?php if ($posts->Export <> "") { ?>
		时间
		<?php } else { ?>
			<a href="postslistsorted.php?order=<?php echo urlencode('datetime') ?>&ordertype=<?php echo $posts->datetime->ReverseSort() ?>">时间<?php if ($posts->datetime->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($posts->datetime->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
		<?php } ?>
				</td>
				<td valign="top">
		<?php if ($posts->Export <> "" ) { ?>
		标题
		<?php } else { ?>
			<a href="postslistsorted.php?order=<?php echo urlencode('title') ?>&ordertype=<?php echo $posts->title->ReverseSort() ?>">网页标题&nbsp;(*)<?php if ($posts->title->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($posts->title->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
		<?php } ?>
				</td>
		<?php if ($posts->Export == "") { ?>
		<?php
		...
		while (!$rs->EOF && $nRecCount < $nStopRec) {
			...
			if (intval($nRecCount) >= intval($nStartRec)) {
				...
				//$postsvalues[$postsvalue] = $postsvalue['countmatch'];
				array_push($postsvalues,$postsvalue);
		...
		?>
			...
			<?php if ($Security->CanDelete()||$Security->CanArchive()) { ?>
			<td nowrap><input type="checkbox" name="ids[]" value="<?php echo $values['id']?>"/></td>
			<?php } ?>
				<!-- id -->
				<td<?php echo $posts->id->CellAttributes() ?>>
					<div<?php echo $posts->id->ViewAttributes() ?>><?php echo $values['id'] ?></div>
				</td>
				<!-- url -->
				<td<?php echo $posts->projectname->CellAttributes() ?>>
					<div<?php echo $posts->projectname->ViewAttributes() ?> class="url"><a href="<?php echo $values['url'] ?>" target="_blank"><?php echo $values['projectname'] ?></a></div>
				</td>
				<!-- datetime -->
				<td<?php echo $posts->datetime->CellAttributes() ?>>
					<div<?php echo $posts->datetime->ViewAttributes() ?>><?php echo $values['datetime'] ?></div>
				</td>
				<!-- title -->
				<td<?php echo $posts->title->CellAttributes() ?>>
					<div<?php echo $posts->title->ViewAttributes() ?>><?php echo $values['title'] ?><?php echo countword( $values['content'],$keywords) ?></div>
				</td>
				<?php if ($Security->CanView()) { ?>
				<td nowrap><span class="phpmaker">
				<a href="<?php echo ViewUrl('postsview.php',intval($values['id']),$keyaction) ?>">快照</a>
				</span></td>
				<?php } ?>
				<?php if ($Security->CanEdit()) { ?>
				<td nowrap><span class="phpmaker">
...
// Load recordset
function LoadRecordsetInSecond($offset = -1, $rowcnt = -1,$lastwhere="") {
	global $conn, $posts;
	...
	// Load list page sql
	$posts->SetCurrentFilter($lastwhere);
	$sSql = $posts->SelectSQL();
...
function count_word($str,$key) {
	...
	// Key url
	function ViewUrl($url,$id, $action = "") {
		$sUrl = $url . "?";
		if ($action <> "") $sUrl .= $action . "&";
		if (!is_null($id)) {
			$sUrl .= "id=" . urlencode($id);
		} else {
