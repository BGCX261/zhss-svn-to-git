...
if (!$Security->IsLoggedIn() ) Page_Terminate("login.php");
...
if (@$_POST["submit"] <> "") {
	...
	if ($sNPwd == $sCPwd) {
		$sFilter = "(`user` = '" . ew_AdjustSql($sUsername) . "')";
	...
	if ($bPwdUpdated) {
		$_SESSION[EW_SESSION_MESSAGE] = "密码修改成功"; // set up message
		Page_Terminate("login.php"); // exit page and clean up
	}
} else {
	$bValidPwd = TRUE;
}
