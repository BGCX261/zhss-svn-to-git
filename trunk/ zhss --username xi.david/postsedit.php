...
<p><span class="phpmaker"><a href="<?php echo $posts->getReturnUrl() ?>">返回</a></span></p>
<?php
...
<input type="text" name="x_projectname" id="x_projectname" title="" size="30" maxlength="255" value="<?php echo $posts->projectname->EditValue ?>"<?php echo $posts->projectname->EditAttributes() ?>>
</span></td>
	</tr>
	<tr class="ewTableAltRow">
		<td class="ewTableHeader">URL</td>
		<td<?php echo $posts->url->CellAttributes() ?>><span id="cb_x_url">
<input type="text" name="x_url" id="x_url" title="" size="30" maxlength="255" value="<?php echo $posts->url->EditValue ?>"<?php echo $posts->url->EditAttributes() ?>>
</span></td>
	</tr>	
	<tr class="ewTableRow">
		<td class="ewTableHeader">日期</td>
		<td<?php echo $posts->datetime->CellAttributes() ?>><span id="cb_x_datetime">
<input type="text" name="x_datetime" id="x_datetime" title="" value="<?php echo $posts->datetime->EditValue ?>"<?php echo $posts->datetime->EditAttributes() ?>>
</span></td>
	</tr>
	<tr class="ewTableAltRow">
		<td class="ewTableHeader">标题</td>
		<td<?php echo $posts->title->CellAttributes() ?>><span id="cb_x_title">
<input type="text" name="x_title" id="x_title" title="" size="30" maxlength="255" value="<?php echo $posts->title->EditValue ?>"<?php echo $posts->title->EditAttributes() ?>>
</span></td>
	</tr>
	<tr class="ewTableRow">
		<td class="ewTableHeader">内容</td>
		<td<?php echo $posts->content->CellAttributes() ?>><span id="cb_x_content">
<textarea name="x_content" id="x_content" cols="135" rows="18"<?php echo $posts->content->EditAttributes() ?>><?php echo $posts->content->EditValue ?></textarea>
</span></td>
	</tr>
</table>
<p>
...
function LoadFormValues() {
	...
	$posts->datetime->CurrentValue = ew_UnFormatDateTime($posts->datetime->CurrentValue, 9);
...
function RenderRow() {
	...
	if ($posts->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		$posts->datetime->EditValue = ew_FormatDateTime($posts->datetime->CurrentValue, 9);
		...
		$posts->counter->EditValue = $posts->counter->CurrentValue;

		// content
		$posts->content->EditCustomAttributes = "";
		$posts->content->EditValue = $posts->content->CurrentValue;
...
function EditRow() {
	...
	if ($rs->EOF) {
		...
		// Field datetime
		$posts->datetime->SetDbValueDef(ew_UnFormatDateTime($posts->datetime->CurrentValue, 9), NULL);
