...
switch ($user->CurrentAction) {
  ...
  case "C": // Copy an existing record
   if (!LoadRow()) { // Load record based on key
      $_SESSION[EW_SESSION_MESSAGE] = "没有找到记录"; // No record found
  ...
  case "A": // ' Add new record
    ...
    if (AddRow()) { // Add successful
      $_SESSION[EW_SESSION_MESSAGE] = "添加记录成功"; // Set up success message
...
<textarea name="x_password" id="x_password" cols="35" rows="4"<?php echo $user->password->EditAttributes() ?>><?php echo $user->password->EditValue ?></textarea>
  ...
  <tr class="ewTableRow">
    <td class="ewTableHeader">权限<span class='ewmsg'>&nbsp;*</span></br>(0代表管理员;其它代表非管理员)</td>
