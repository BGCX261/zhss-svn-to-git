...
if( $action != '' ) {
	...
	if( $action == 'delete' ) {
		...
		$idstring = implode('","',$ids);
		$sql = 'SELECT h.*,p.content,p.title FROM `hoturls` h,`posts` p WHERE (h.`id` IN ("' . $idstring . '")) and (p.`url` = h.`url`)';
		echo $sql;
		$rs = $conn->Execute($sql);
		while(!$rs->EOF) {
...
function BasicSearchSQL($Keyword) {
	...
	$sql = "";
	$sql .= "h.`url` LIKE '%" . $sKeyword . "%' OR ";
	$sql .= "h.`categories` LIKE '%" . $sKeyword . "%' OR ";
	$sql .= 'p.`title` LIKE "%' . $sKeyword . '%" OR p.`content` LIKE "%' . $sKeyword . '%" ';
