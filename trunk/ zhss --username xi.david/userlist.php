...
<table class="ewBasicSearch">
	<tr>
		...
		<td>
			<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" size="10" class="searchInput" value="<?php echo $user->getBasicSearchKeyword() ?>">
		</td>
	</tr>
	<tr>
		<td colspan="2">
...
id
<?php } else { ?>
	<a href="userlist.php?order=<?php echo urlencode('id') ?>&ordertype=<?php echo $user->id->ReverseSort() ?>">编号<?php if ($user->id->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($user->id->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</td>
		<td valign="top">
<?php if ($user->Export <> "") { ?>
user
<?php } else { ?>
	<a href="userlist.php?order=<?php echo urlencode('user') ?>&ordertype=<?php echo $user->user->ReverseSort() ?>">用户名&nbsp;(*)<?php if ($user->user->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($user->user->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</td>
		<td valign="top">
<?php if ($user->Export <> "") { ?>
password
<?php } else { ?>
	<a href="userlist.php?order=<?php echo urlencode('password') ?>&ordertype=<?php echo $user->password->ReverseSort() ?>">密码&nbsp;(*)<?php if ($user->password->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($user->password->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</td>
		<td valign="top">
<?php if ($user->Export <> "") { ?>
level
<?php } else { ?>
	<a href="userlist.php?order=<?php echo urlencode('level') ?>&ordertype=<?php echo $user->level->ReverseSort() ?>">权限(0-管理员)<?php if ($user->level->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($user->level->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</td>
<?php if ($user->Export == "") { ?>
...
<div<?php echo $user->password->ViewAttributes() ?>>************</div>
</td>
		<!-- level -->
...
<a href="<?php echo $user->DeleteUrl() ?>">删除</a>
</span></td>
<?php } ?>
<?php } ?>
	</tr>
...
function RenderRow() {
	...
	if ($user->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		if (!is_null($user->user->ViewValue)) $user->user->ViewValue = str_replace("\n", "<br>", $user->user->ViewValue); 
		$user->user->CssStyle = "";
		$user->user->CssClass = "";
		...
		if (!is_null($user->password->ViewValue)) $user->password->ViewValue = str_replace("\n", "<br>", $user->password->ViewValue); 
